...
from utils.losses import KLDivLoss
...
def save_best_model(model, s, best_metric, now_metric):
    if now_metric > best_metric:
        ...
        torch.save(model.state_dict(), f"checkpoints/Prostate_cosin_buffer/{s}_cosin_best_model.pth")
    return best_metric


def log_epoch_results(train_log, epoch, epoch_loss, train_dice, epoch_loss_test, test_dice, best_metric,
                      buffer1_dice=None, buffer2_dice=None, epoch_klloss=None):
    log_message = (
    ...
    if epoch_klloss:
        log_message += (
            f" --train KL loss:{round(epoch_klloss, 6)} --Buffer Dice:{round(np.mean(buffer1_dice), 6)}/{round(np.mean(buffer2_dice), 6)}")
...
def train_first_site(model, s, trainloader, testloader, epochs, device, root, i_list, t_list):
    ...
    best_metric = -1
    train_log = open(f'logs/Prostate_cosin_buffer/{s}_cosin_buffer.log', 'w')
...
def train_other_sites(model, s, trainloader, testloader, bufferloader, epochs, device, root, i_list, t_list):
    ...
    for epoch in range(epochs):
        model.train()
        running_loss, running_loss_kl = 0, 0
        ...
        for x, y, img_buffer, lab_buffer in tqdm(trainloader):
            ...
            loss = loss1 + loss2 + loss3 + 0.0005 * losskl
        ...
        log_epoch_results(train_log, epoch, epoch_loss, train_dice, epoch_loss_test, test_dice, best_metric,
                          buffer1_dice, buffer2_dice, epoch_klloss)
